...
def zero_row_min(x):
  ...
  M, N = x.shape
  y = x.clone()
  y[torch.arange(M), x.argmin(dim=1)] = 0
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop:
    z = torch.stack([torch.matmul(x_mat, y_mat) for x_mat, y_mat in zip(x, y)])
  else:
    z = torch.bmm(x, y)
...
def normalize_columns(x):
  ...
  means = x.sum(dim=0) / M
  squares = (x - means) ** 2
  sigma = (squares.sum(dim=0) / (M - 1)) ** 0.5
  y = (x - means) / sigma
